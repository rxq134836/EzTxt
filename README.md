...
- **完成自动归档**：勾选完成的任务自动移入归档（独立归档窗口按日期分组展示），可恢复为待办
...
- **动画小球**：小球可切换为 GIF 动画风格（内置蕾米埃尔主题 GIF：闲时轮换 + 打字速度反馈），也可上传 GIF 制作**自定义动画主题**（闲时 / 慢速打字 / 快速打字），动画模式隐藏数字角标、纯 GIF 球体
- 小球固定经典实心主题色，不随材质切换
...
- **开机自启**：Windows 登录时自动启动 EzTxt（设置 → 系统）
...
2. **完成 / 删除**：点击卡片左侧勾选完成，任务自动飞入**归档**；「删除」进入批量模式，勾选多张后二次确认删除
3. **备注**：点击卡片右侧「展开备注」箭头，在编辑区输入 Markdown，内容所见即所得
4. **分割线**：工具栏「分割」插入分组线，可给分组起标题
5. **归档**：工具栏「归档」按钮（带数量角标）打开已完成任务窗口，可按日期查看、点击卡片查看备注、批量删除；鼠标移入卡片右侧滑出「恢复待办」按钮可放回待办
6. **收缩为小球**：工具栏「缩为小条」，窗口收缩为桌面悬浮球（拖动到屏幕边缘也可自动贴边）
7. **设置**：工具栏「设置」打开独立设置窗口
...
| `Backspace` | 删除选中任务（非输入态，二次确认） |
| `Ctrl+Z` | 撤销备注编辑（自定义快照栈，含光标位置恢复） |
| `Ctrl+Shift+Z` / `Ctrl+Y` | 重做备注编辑 |
| `Ctrl+/` | 显示 / 隐藏快捷键帮助 |
...
```
easy-txt/
├── CLAUDE.md                # AI 行为准则（Karpathy Guidelines，改代码前必读）
├── src/                     # 应用源码
│   ├── main.js              # Electron 主进程：窗口/托盘/贴边/存储/IPC/归档/动画主题
│   ├── preload.js           # preload：marked/turndown/highlight.js 封装 + contextBridge
│   ├── renderer.js          # 主窗口渲染逻辑：待办列表 + WYSIWYG 编辑器 + 归档动画
│   ├── index.html           # 主窗口页面
│   ├── settings.html        # 独立设置窗口页面
│   ├── settings.js          # 设置窗口逻辑
│   ├── archive.html / archive.js    # 已完成任务归档窗口（按日期分组、恢复/批量删除）
│   ├── custom-theme.*       # 自定义主题调色窗口（html/js）
│   ├── custom-gif-theme.*   # 自定义 mini 动画主题编辑器（html/js）
│   ├── styles.css           # 全局样式（主题/材质/编辑器/滚动条/归档卡片等）
│   ├── mini-gifs/           # mini 球内置动画 GIF（remi-1~4 闲时轮换 / remi-slow / remi-fast）
...
├── docs/
│   └── UI-DESIGN-SPEC.md    # UI 设计规范（改 UI 前必读）
├── data/                    # 开发模式数据目录（自动生成）
├── dist/                    # 打包输出目录（自动生成）
├── .npmrc                   # npm 镜像配置
└── package.json
...
| `miniBallStyle` | mini 球风格：`classic`（经典圆球）/ `gif`（动画） |
| `miniBallGif` | mini 球动画主题名（如 `remi`；GIF 按命名规范自动扫描发现，支持自定义主题） |
| `autoStart` | 开机自启（Windows 登录时启动） |
...
**编辑器撤销/重做**
- 备注编辑框新增 **`Ctrl+Z` 撤销** / **`Ctrl+Shift+Z`（或 `Ctrl+Y`）重做**，自定义快照栈管理（每编辑器实例独立，最多 50 步）
- 撤销/重做时**自动恢复光标位置**到操作时的偏移量，不再跳到首行
- 格式化操作（加粗 / 删除线 / 代码块等）后立即压栈，连续打字防抖 600ms 合并为一步

**删除确认**
- `Backspace` 删除待办项时新增**二次确认弹窗**，显示任务标题，支持 `Enter` 确认 / `Esc` 取消

**开机自启**
- 新增**开机自启**选项（设置 → 系统）：Windows 登录时自动启动 EzTxt，默认关闭

**Mini 球打字状态 GIF**
- Mini 球动画模式下新增**全局键盘监听**（主进程 PowerShell `GetAsyncKeyState`），即使焦点在其他应用也能检测打字
- 打字慢速（按键间隔 > 450ms）→ 切换 `remi-5.gif`；快速持续（间隔 < 250ms 且持续 > 1.5 秒）→ 切换 `remi-6.gif`；暂停 2 秒后恢复轮换
- PowerShell 进程常驻（app 启动即启动），逐键边沿检测 + `readline` 逐行解析，进程崩溃 3 秒自动重启

**GIF 显示修复**
- 修复不同宽高比 GIF（竖向 remi-1~4 / 正方形 remi-5 / 横向 remi-6）切换时视觉大小不一致的问题，`background-size` 从 `cover` 改为 `100% 100%` 强制拉伸

### 2026-09-04

**Mini 球动画主题化**
- Mini 球动画不再限制在蕾米埃尔主题：任意主题下开启「动画」风格均可用 GIF
- 新增**动画主题选择器**：从 `mini-gifs/` 目录自动扫描并选择内置主题（当前为 remi）
- 打字状态 GIF 文件名规范化：`remi-5.gif` → `remi-slow.gif`（慢打字）、`remi-6.gif` → `remi-fast.gif`（快打字）

**自定义 GIF 动画主题**
- 新增**自定义 mini 动画主题**：独立编辑窗口上传 GIF，可配置多张闲时轮换动画 + 慢速/快速打字动画（各一张）
- 自定义主题保存到数据目录 `mini-gif-custom/`（GIF 文件 + `custom-gif-themes.json` 元数据），可编辑、可删除
- 主题选择器支持**双击编辑 / 删除**自定义主题，保存后所有窗口即时刷新

**已完成任务归档**
- 勾选完成的任务**自动归档**：卡片飞入「归档」按钮动画后从待办列表移入归档
- 新增独立**归档窗口**（主界面「归档」按钮打开）：已完成任务按完成日期分组展示（今天 / 昨天 / N 天前）
- 归档窗口支持**取消归档**（放回待办）、**批量选择删除**（带二次确认弹窗）
- 取消归档交互：鼠标移入归档卡片时右侧滑出「恢复待办」按钮，点击才恢复（整卡点击不再误触）
- 归档卡片**点击收起区（标题行/时间区）展开或收起**只读 Markdown 备注；已展开的备注正文可选中文本、不触发收起
- 归档卡片标题行**满宽布局**：标题靠左、完成时间靠右，不受备注长度影响

**UI 规范补充**
- `docs/UI-DESIGN-SPEC.md` 新增 **§11 危险/删除操作按钮样式约定**：所有删除/确定删除按钮统一使用 `.tb-btn.danger`，禁止自创新危险类名

### 2026-09-22

**数据备份**
- 设置页「数据保存」下新增**数据备份**：一键导出 / 导入备份
- **导出备份**：把当前待办数据（`note.json`）、设置（`settings.json`）与自定义动画主题（`mini-gif-custom/`）打包成 zip，选择保存位置即可
- **导入备份**：选择备份 zip 恢复数据，校验包内 `note.json` 后覆盖回当前存储目录；导入前弹独立确认框，提示将覆盖现有数据
- 导出 / 导入按钮采用与「关闭按钮」栏目一致的高亮选中样式（`material-btn.is-active`），随主题色变化
---
